...
- **[AutoSwap](https://github.com/fino-oss/autoswap)** – Cross-chain swaps in **one function call** for AI agents. Auto-routes via Paraswap/Uniswap V3, bridges via Across Protocol (~2s fills), resolves cold-start gas automatically, and enforces slippage protection (never sets amountOutMin=0). Supports Base, Polygon, Arbitrum, Optimism. Dry-run simulation built-in. Available as `pip install autoswap` and `npm install autoswap`.
---
